...
-- Task 3.3
SELECT 
    FORMAT('Project: %s - Budget: $%.2f - Status: %s', project_name, budget, status) AS project_info
FROM projects;
...
SELECT 
    ...
    GREATEST(MAX(salary), 70000) AS salary_vs_70k,
    LEAST(MIN(salary), 60000) AS salary_vs_60k
FROM employees e1
GROUP BY department;
